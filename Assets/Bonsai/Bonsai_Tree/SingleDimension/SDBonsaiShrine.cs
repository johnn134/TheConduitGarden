...
public class SDBonsaiShrine : MonoBehaviour {
	...
	void checkTrees() {
		...
		for(int i = 0; i < trees.Length; i++) {
			...
			if(g.GetComponent<SDBonsaiManager>() != null) {
				//Overgrown check
				if(g.GetComponent<SDBonsaiManager>().getNumLeaves() >= g.GetComponent<SDBonsaiManager>().maxLeaves ||
					g.GetComponent<SDBonsaiManager>().getNumBranches() >= g.GetComponent<SDBonsaiManager>().maxBranches) {
					issues[i] += 1;
				}

				//Infestation check
				if(g.GetComponent<SDBonsaiManager>().getNumInfestedBranches() > 0) {
					issues[i] += 1;
				}

				//Dead leaves and branches check
				if(g.GetComponent<SDBonsaiManager>().getNumDeadLeaves() > 0 || 
					g.GetComponent<SDBonsaiManager>().getNumDeadBranches() > 0) {
					issues[i] += 1;
				}

				//Undergrowth check
				if(g.GetComponent<SDBonsaiManager>().getNumLeaves() < MIN_LEAVES ||
					g.GetComponent<SDBonsaiManager>().getNumBranches() < MIN_BRANCHES) {
	...
	bool checkTokyoContractProgress() {
		...
		foreach(GameObject tree in trees) {
			int[] reqZonePasses = tree.GetComponent<SDBonsaiManager>().getReqZonePasses();
			if(tree.GetComponent<SDBonsaiManager>().getZoneExtensions() > 0 ||
				reqZonePasses[0] <= 0 ||
				reqZonePasses[1] <= 0 ||
				reqZonePasses[2] <= 0 ||
				reqZonePasses[3] <= 0 ||
