...
public class SDBud : MonoBehaviour {
	...
	void OnDestroy() {
		...
		if(transform.parent != null) {
			if(transform.parent.GetComponent<SDBranch>() != null) {
				transform.parent.GetComponent<SDBranch>().registerBudRemoved();
	...
	public void processGrowthCycle() {
		if(isLeaf) {
			GameObject newLeaf = Instantiate(Resources.Load("Bonsai/SDLeafPrefab"), Vector3.zero, Quaternion.identity, transform.parent) as GameObject;
			//Set transform
			...
			newLeaf.GetComponent<SDLeaf>().setManager(manager);
			newLeaf.GetComponent<SDLeaf>().checkIfLeafSatisfiesContract();

			//Set w position
			newLeaf.GetComponent<HyperColliderManager>().setW(transform.GetChild(0).GetComponent<HyperObject>().w);

			//Register Leaf Added
			transform.parent.GetComponent<SDBranch>().registerLeafAdded();
		}
		else {
			GameObject newBranch = Instantiate(Resources.Load("Bonsai/SDBranchPrefab"), Vector3.zero, Quaternion.identity, transform.parent) as GameObject;
			...
			//Initialize Variables
			newBranch.GetComponent<SDBranch>().setDepth(depth);
			newBranch.GetComponent<SDBranch>().setManager(manager);
			newBranch.GetComponent<SDBranch>().checkIfBranchSatisfiesContract();

			//Set w position
			newBranch.GetComponent<HyperColliderManager>().setW(transform.GetChild(0).GetComponent<HyperObject>().w);

			//Register Branch Added
			transform.parent.GetComponent<SDBranch>().registerBranchAdded();
		}
