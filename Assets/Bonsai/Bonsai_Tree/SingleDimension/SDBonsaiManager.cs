...
public class SDBonsaiManager : MonoBehaviour {
	...
	void Start () {
		...
		baseBranch = Instantiate(Resources.Load("Bonsai/SDBranchPrefab"), transform) as GameObject;
		baseBranch.transform.localPosition = Vector3.zero;
		baseBranch.transform.localScale = new Vector3(treeSize, treeSize, treeSize);
		baseBranch.GetComponent<SDBranch>().setcanSnip(false);
		baseBranch.GetComponent<SDBranch>().setDepth(0);
		baseBranch.GetComponent<SDBranch>().setManager(this.gameObject);
		baseBranch.GetComponent<SDBranch>().checkIfBranchSatisfiesContract();

		baseBranch.GetComponent<HyperColliderManager>().setW(w);

		baseBranch.GetComponent<SDBranch>().setupTreeForLevel(levelType);
	...
	void processGrowthCycle() {
		baseBranch.GetComponent<SDBranch>().processGrowthCycle();
	}
