...
public class SteamVR_ControllerManager : MonoBehaviour
{
	...
	private void OnDeviceConnected(params object[] args)
	{
		...
		if (connected)
		{
			...
			if (system != null && system.GetTrackedDeviceClass(index) == ETrackedDeviceClass.Controller)
			{
				...
				for(int i = 0; i < objects.Length; i ++){
					Debug.Log("LOOKING: " + indices[i]);
					if(indices[i] == index){
						thisObject = objects[i];
						Debug.Log("FOUND : " + objects[i].gameObject.name);
					}
				}
				if(thisObject){
					if(l == index){ //&& gameObject.name.Equals("ControllerL")){
						thisObject.gameObject.tag = "LeftControl";
						Debug.Log("LEFT: " + thisObject.gameObject.name);
						//controlInfo = GameObject.Find("LeftControlInfo");
						//controlInfo.GetComponent<TextMesh>().text = "Connected";
					}
					else{
						thisObject.gameObject.tag = "RightControl";
						Debug.Log("RIGHT: " + thisObject.gameObject.name);
						//controlInfo = GameObject.Find("RightControlInfo");
						//controlInfo.GetComponent<TextMesh>().text = "Connected";
					}
				}*/
			}
	...
	public void Refresh()
	{
		...
		else
		{
			...
			if (leftIndex != OpenVR.k_unTrackedDeviceIndexInvalid && rightIndex != OpenVR.k_unTrackedDeviceIndexInvalid)
			{
				//but first set the tags
				var l = SteamVR_Controller.GetDeviceIndex(SteamVR_Controller.DeviceRelation.Leftmost);
				//var r = SteamVR_Controller.GetDeviceIndex(SteamVR_Controller.DeviceRelation.Rightmost);
				//Debug.Log((l == r) ? "first" : (l == leftIndex) ? "left" : "right");

				//left
				GameObject leftCtrl = null;
				for(int i = 0; i < objects.Length; i ++){
					if(indices[i] == leftIndex){
						leftCtrl = objects[i];
					}
				}
				if(leftCtrl){
					if(l == leftIndex){ //&& gameObject.name.Equals("ControllerL")){
						leftCtrl.gameObject.tag = "LeftControl";
						//Debug.Log("LEFT: " + leftCtrl.gameObject.name);
						//controlInfo = GameObject.Find("LeftControlInfo");
						//controlInfo.GetComponent<TextMesh>().text = "Connected";
					}
					else{
						leftCtrl.gameObject.tag = "RightControl";
						//Debug.Log("RIGHT: " + leftCtrl.gameObject.name);
						//controlInfo = GameObject.Find("RightControlInfo");
						//controlInfo.GetComponent<TextMesh>().text = "Connected";
					}
				}

				l = SteamVR_Controller.GetDeviceIndex(SteamVR_Controller.DeviceRelation.Leftmost);
				//r = SteamVR_Controller.GetDeviceIndex(SteamVR_Controller.DeviceRelation.Rightmost);
				//Debug.Log((l == r) ? "first" : (l == rightIndex) ? "left" : "right");

				//right
				GameObject rightCtrl = null;
				for(int i = 0; i < objects.Length; i ++){
					if(indices[i] == rightIndex){
						rightCtrl = objects[i];
					}
				}
				if(rightCtrl){
					if(l == rightIndex){ //&& gameObject.name.Equals("ControllerL")){
						rightCtrl.gameObject.tag = "LeftControl";
						//Debug.Log("LEFT: " + rightCtrl.gameObject.name);
						//controlInfo = GameObject.Find("LeftControlInfo");
						//controlInfo.GetComponent<TextMesh>().text = "Connected";
					}
					else{
						rightCtrl.gameObject.tag = "RightControl";
						//Debug.Log("RIGHT: " + rightCtrl.gameObject.name);
						//controlInfo = GameObject.Find("RightControlInfo");
						//controlInfo.GetComponent<TextMesh>().text = "Connected";
					}
				}
